...
每个[呼叫端](./呼叫链5843921.mdx#0-关于)（通道）的定义是FreeSWITCH与其他设备之间的连接。这个其他设备可以是VoIP电话、PRI连接，甚至是计算机上的音频设备。每种类型的设备都有自己的协议用于设置通道、协商[编解码器](../编解码器与媒体/index.mdx#关于)、发送和接收媒体。在FreeSWITCH中，通道本身与特定协议无关，而是使用终节点模块实现协议。这种架构使我们能够使用不同的协议来桥接通道。它也使我们能够轻松实现新的协议。
...
VoIP最流行的信令协议是SIP，在FreeSWITCH中我们使用[Sofia SIP栈](../配置/Sofia-SIP栈/index.mdx#常见问题)。将Sofia与FreeSWITCH关联起来的终节点模块是[mod\_sofia](../模块/mod_sofia_1048707.mdx#关于)。
